...
async function spawnChild(
    ...
    const child = spawn(command, args, {
        stdio: [
            inputFilePath ? fs.openSync(inputFilePath, "r") : "ignore",
            out,
            "pipe",
        ],
    });
...
async function judge(problemid, file) {
    ...
    if (exitCode !== 0) {
        ...
        for (const file of files) {
            const out = `${tmpDir}/${file.filename}_${file}.out`,
                inp = `${testDir}/${file}`;
            ...
            if (exitCode !== 0) {
                ...
                const { exitCode, err } = await spawnChild(
                    problemDir + "/checker",
                    [inp, out]
                );
                console.log(
                    `test: ${file}, exitcode: ${exitCode}, error: ${err}`
                );
                if (exitCode !== 0) {
                    console.error(err);
                    correct = false;
